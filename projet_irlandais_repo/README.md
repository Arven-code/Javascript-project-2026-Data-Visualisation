...
This project starts from that difficulty. It uses Census 2022 data to produce a county-level choropleth map of Irish speakers as a percentage of the total population. Its primary object is therefore not the Irish language as a whole, but one precise statistical variable: the percentage of people recorded in the census as Irish speakers in each county-level unit, and Electoral Divisions. The map translates that variable into colour, while the side panel gives a second level of reading by showing a short ability breakdown for the selected county, and later for the ispopleth-style map, for the single Electoral Divisions.

The project hence contains two maps, ome built at the Electoral Division scale, an exploratory isopleth and one built out of a choropleth style visualisation for the county units. The isopleth map uses smaller territorial units and then smooths the values into a continuous colour surface, making the distribution more local and visually more fluid, whilst also making the question of interpretation more delicate. For that reason, the isopleth keeps the smoothed surface and the original Electoral Division value separate: the colour surface is interpolated, while the reading box and the side panel give the nearest original ED value.
...
Both maps allow for a different way of asking the same spatial question to be formulated. The choropleth keeps the county-level statistical unit more visible, whilst the isopleth moves closer to the Electoral Division data and makes a smoother exploratory surface, and allows for a more fine-grained view of the language level variations throughout Ireland. The separate `iso/index_iso.html` file is kept in the repository as a backup version, but the archived interface is now the switch inside the main page.
